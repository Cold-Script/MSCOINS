...
local Toggle = Tab:Toggle("Entity Notifier",function(Value) if Value then EntityNotifier = workspace.ChildAdded:Connect(function(child) task.wait(1) if child.Name == "Eyes" then notify("Eyes") if EntityEsp then selection(child, "[Eyes]") end 
elseif child.Name == "FigureLibrary" then if EntityEsp then selection(child, "[Figure]") end elseif child.Name == "FigureRagdoll" then if EntityEsp then selection(child, "[Figure]") end elseif child.Name == "SeekMoving" then 
notify("Seek")
...
local Toggle = Tab2:Toggle("Closet ESP",function(Value) if Value then for _, v in pairs(workspace.CurrentRooms:GetDescendants()) do if v.Name == "Wardrobe" or "RetroWardrobe" or "Rooms_Locker" or "Backdoor_Wardrobe" and v:IsA("Model") then selection(v, "Closet", false, true) end end ClosetESP = workspace.CurrentRooms.ChildAdded:Connect(function(child) task.wait(1) for _, v in pairs(child:GetDescendants()) do if v.Name == "Wardrobe" or "RetroWardrobe" or "Rooms_Locker" or "Backdoor_Wardrobe" and v:IsA("Model") then selection(v, "Closet", false, true) end end end) else ClosetESP:Disconnect() for _, v in pairs(workspace.CurrentRooms:GetDescendants()) do if v.Name == "KiwiHighlight_3" then v:Destroy() end end end end )

local Toggle = Tab2:Toggle("Entities ESP",function(Value) EntityEsp = Value end )
